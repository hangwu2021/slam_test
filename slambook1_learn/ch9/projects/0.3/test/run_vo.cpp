...
int main(int argc, char* argv[])
{
    ...
    std::cout << "camera.depth_scale: " << camera->depth_scale_ << std::endl;
    
    // Read Trajectory
    std::vector<Eigen::Isometry3d, Eigen::aligned_allocator<Eigen::Isometry3d>> poses;
    std::ifstream fin("data/trajectory.txt");
    if (!fin) {
        std::cout << "cannot find trajectory file at data/trajectory.txt" << std::endl;
        return -1;
    }
    ...
    std::cout << poses[10].translation() << std::endl;
